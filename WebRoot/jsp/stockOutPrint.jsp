...
<html> 
	...
	<script type="text/javascript">
		...
		$().ready(function() {
			...
			alert(67);
			alert($("#aa").html());
			alert($("#wholeQuantity",window.parent.document).html());
	...
	<body >
		...
		<html:form styleId="stockOut" action="/stockOut.do?method=verify" >
			
		<div >
		 	<h1 style="font-size: 16px; text-align: center; padding:0px;margin: 0px">硕春冷库出库单</h1>
  			<table class="tableHead" width="100%">
	  			<tr>
					<td width="33%">对方：<c:out value="${stockOutForm.customerName}" /></td>
					<td width="33%">出库日期：<c:out value="${stockOutForm.time}"/></td>
					<td width="33%">单号：<c:out value="${stockOutForm.serialNumber}" /></td>
				</tr>
			</table>
		</div>
		<div >
		<table cellpadding="0" cellspacing="0" border="0" class="tableMain"  width="100%" >
			...
			<c:forEach items="${requestScope['cargoes']}" var="cargo" varStatus="status">
